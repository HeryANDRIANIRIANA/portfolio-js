class pointageBE{
	...
	async addRowPointage2(options={}){
		const{ar=[], structure={}}=options
		try{
			// console.log(ar);
			// let o={structure:structure};
			
			let r=this.Qman.addRow({ar:ar,structure:structure,tableKey:"idPointage2",tableName:"POINTAGE2", saveInJson:true} )
			return r;
		}catch(err){
			console.log(err)
		}
	...
	async updtRowPointage2(options={}){
		const{rows=[], tableName="", id="" }=options
		try{
			let r=this.Qman.updtRow({rows:rows, tableName:tableName, id:id })
			return r
		}catch(err){
			console.log(err)
		}
	...
	async saveInJson2(opt={}){
		const{}=opt
		let o=opt.pointageById;
		let fName=`public/data/${opt.fileName}.json`
		let r=this.Qman.sauvegarderObjetJSON(o, fName)
		// console.log(opt);
	}

	async getPointage(opt={}){
		const{}=opt
	}
}
